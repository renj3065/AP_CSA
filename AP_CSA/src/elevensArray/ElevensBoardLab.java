 ...
 */
public class ElevensBoardLab extends Board{
...
//System.out.println(selectedCards.size()==2 );
...
if (selectedCards.size()==2 && containsFacePair(selectedCards)){
	//System.out.println("Mark 2");
	return true;
}
if (selectedCards.size()==3 && sum11(selectedCards)){
	//System.out.println("Mark 2");
	return true;
}
return false;	
	...
	public boolean anotherPlayIsPossible() {
		...
		if(containsFacePair(cardIndexes())){
			return true;
		}
		if(containsPairSum11(cardIndexes()) && hasFace(cardIndexes())){
			return true;
		}
		return false;
	}
...
return false;
	...
	private boolean containsFacePair(List<Integer> selectedCards){
		...
		boolean hasQueen=false;
		boolean hasKing=false;
		boolean hasJack2=false;
		boolean hasQueen2=false;
	...
	private boolean sum11(List<Integer> selectedCards){
		if(selectedCards.size()==3){
		int sum=0;
		for(int x: selectedCards){
			sum+=cardAt(x).pointValue();
		}
		if(sum==11){
			return true;
		}
	...
	private boolean hasFace(List<Integer> selectedCards){
		boolean hasFace=false;
		for(int x: selectedCards){
			if(cardAt(x).pointValue()==0){
				hasFace=true;
			}
		}
		return hasFace;
